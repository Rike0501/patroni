...
from patroni.postgresql.mpp.citus import CitusMultiHandler, PgDistGroup, PgDistNode
...
class TestCitus(BaseTestPostgresql):
    ...
    @patch.object(CitusMultiHandler, 'is_alive', Mock(return_value=True))
    def test_run(self, mock_logger_warning):
        ...
        self.c.add_task(self.database2, 'after_promote', 2, self.cluster, self.cluster.leader_name, 'postgres://host3:5432/postgres')
    ...
    @patch.object(CitusMultiHandler, 'is_alive', Mock(return_value=False))
    @patch.object(CitusMultiHandler, 'start', Mock())
    def test_sync_meta_data(self):
        with patch.object(CitusMultiHandler, 'is_enabled', Mock(return_value=False)):
            self.c.sync_meta_data(self.cluster)
        self.c.sync_meta_data(self.cluster)

    def test_handle_event(self):
        self.c.handle_event(self.cluster, {})
        with patch.object(CitusMultiHandler, 'is_alive', Mock(return_value=True)):
    ...
    def test_pick_task(self):
        ...
        with patch.object(CitusMultiHandler, 'update_node') as mock_update_node:
    ...
    def test_process_task(self):
        ...
        task = self.c.add_task(self.database, 'after_promote', 1, self.cluster,
                               self.cluster.leader_name, 'postgres://host4:5432/postgres', 30)
        with patch.object(CitusMultiHandler, 'query') as mock_query:
            self.c.process_tasks(self.database)
    ...
    def test_process_tasks(self):
        ...
        with patch('patroni.postgresql.mpp.citus.logger.error') as mock_logger, \
                patch.object(CitusMultiHandler, 'query', Mock(side_effect=Exception)):
            self.c.process_tasks(self.database)
            mock_logger.assert_called()
